...
func TestAuthorized(t *testing.T) {
	...
	tests := []struct {
		...
		{"member of the admin group", "jane@example.com", []string{"eng", "dex-admins"}, true},
		{"break-glass email, case insensitive", "break.glass@example.com", nil, true},
		{"authenticated but not an admin", "john@example.com", []string{"eng"}, false},
		{"no groups at all", "john@example.com", nil, false},
		{"group name is not a substring match", "john@example.com", []string{"dex-admins-readonly"}, false},
		{"empty email must not match an empty allow entry", "", nil, false},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			if got := a.authorized(tc.email, tc.groups); got != tc.want {
				t.Errorf("authorized(%q, %v) = %v, want %v", tc.email, tc.groups, got, tc.want)
			}
		})
	}
}
